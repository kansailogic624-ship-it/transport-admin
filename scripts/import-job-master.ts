/**
 * 業務マスタ.xlsx を Firestore jobs へ一括取り込み
 *
 * 使い方:
 *   npm run import:jobs
 *   npm run import:jobs -- "C:/path/to/業務マスタ.xlsx"
 */
...
import { userJobsPath } from "../src/services/firestore-paths";
...
async function replaceJobDetails(uid: string, jobs: JobDetail[]): Promise<void> {
  const collectionPath = userJobsPath(uid);
  ...
  const ops: Array<
    ...
    | { type: "set"; ref: ReturnType<typeof doc>; data: JobDetail }
  > = [
    ...
    ...jobs.map((job) => ({
      type: "set" as const,
      ref: doc(colRef, job.id),
      data: cleanForFirestore(job),
    })),
  ];
...
async function main(): Promise<void> {
  ...
  if (jobs.length === 0) {
    console.error("取り込める業務データがありません。");
    for (const w of warnings) console.error("  ⚠", w);
    process.exit(1);
  }

  const user = await signIn();
  await replaceJobDetails(user.uid, jobs);

  console.log(`✓ ${jobs.length} 件を users/${user.uid}/jobs へ保存しました`);
